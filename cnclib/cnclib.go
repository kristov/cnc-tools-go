...
func polyfillPoint(rst *PolyFillRaster, x, y int32) orb.Point {
    return orb.Point{zify(float64(x) * rst.Conv),zify(float64(y) * rst.Conv)}
}

func polyfillCanMove(rst *PolyFillRaster, x, y int32, dx, dy int8) bool {
    if dx < 0 {
        if x == 0 {
            return false
        }
...
func polyfillMarkAppend(rst *PolyFillRaster, ls orb.LineString, x, y int32) orb.LineString {
...
func PolyFill(ls orb.LineString, toolrad float64) orb.MultiLineString {
    ...
    rst.Sx = int32(math.Round((max[0] - min[0]) / line_sep))
    rst.Sy = int32(math.Round((max[1] - min[1]) / line_sep))
    rst.Conv = line_sep
    rst.Raster = make([]uint8, rst.Sx * rst.Sy)
    var y, x int32
    for y = 0; y < rst.Sy; y++ {
        for x = 0; x < rst.Sx; x++ {
